...
function toggleRow(id: string) {
	expandedRows[id] = !(expandedRows[id] ?? probes.length === 1);
}
...
<section
	id="probe-region-details"
	class="order-4 rounded-lg border border-neutral-800 bg-neutral-900/10 p-4 lg:order-none lg:col-span-2"
	aria-labelledby="probe-region-details-title"
>
	<div class="mb-3 flex items-center justify-between gap-3">
		<div>
			<h4
				id="probe-region-details-title"
		...
		<button
			type="button"
			class="rounded border border-neutral-700 px-2 py-1 text-xs text-neutral-300 hover:bg-neutral-800 focus-visible:outline-2 focus-visible:outline-primary"
			onclick={onClose}
		>
			Закрыть
		</button>
			...
			<table class="w-full text-left text-sm border-collapse">
				...
				<tbody>
					{#each probes as probe (probe.probe_id)}
						...
						{@const isExpanded = expandedRows[probe.probe_id] ?? probes.length === 1}
						<tr
	...
	{:else}
		<p class="text-sm text-neutral-500">Нет ответов сканеров</p>
	{/if}
</section>
