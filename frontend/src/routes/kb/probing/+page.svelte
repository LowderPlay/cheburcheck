...
<KbArticle>
	...
	<p>
		Динамический результат показывает карту регионов рядом со списком вердиктов
		и числом сканеров для каждого признака блокировки. Наведите на регион, чтобы
		увидеть число ответов и вердикты. Каждый контрольный CDN-сервер показан
		отдельным сегментом цветной полосы. Также видны результаты DNS-провайдеров и
		трассировки IP. Один сканер может сообщить сразу о нескольких признаках.
	</p>
	<ul class="list-disc">
		<li>
			<b>Доступен</b>
